...
export function PrayerGrid({
  ...
  const handleSelect = (row: string, colValue: string) => {
    onChange({ ...value, [row]: colValue });
  };
    ...
    <div className="w-full space-y-4">
      ...
      <div className="md:hidden space-y-4">
        {rows.map((row) => (
          ...
          >
            <h3 className="font-bold text-lg text-primary">{row}</h3>
            <div className="grid grid-cols-2 gap-2">
              {columns.map((col) => {
                const isSelected = value[row] === col.value;
                return (
                  <button
                    key={col.value}
                    type="button"
                    onClick={() => handleSelect(row, col.value)}
                    className={cn(
          ...
          <tbody className="divide-y">
            {rows.map((row) => (
              <tr key={row} className="hover:bg-muted/20 transition-colors">
                <td className="p-4 font-bold text-primary w-1/5">{row}</td>
                {columns.map((col) => {
                  const isSelected = value[row] === col.value;
                  return (
