 ...
 * Circle Quests Page
 *
 * Teacher dashboard page for managing custom quests scoped to their circle.
 * Teachers can create, edit, and delete quests that only their students see.
 */
...
import {
  useTeacherQuests,
  useCreateTeacherQuest,
  useUpdateTeacherQuest,
  useDeleteTeacherQuest,
  type TeacherQuest,
} from "@/hooks/use-teacher-quests";
...
export default function CircleQuestsPage() {
  const t = useTranslations("CircleQuests");
  const { data: quests = [], isLoading } = useTeacherQuests();
  const createQuest = useCreateTeacherQuest();
  const updateQuest = useUpdateTeacherQuest();
  const deleteQuest = useDeleteTeacherQuest();
  const { toast } = useToast();

  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingQuest, setEditingQuest] = useState<TeacherQuest | null>(null);
  ...
  const handleOpenEdit = (quest: TeacherQuest) => {
  ...
  const columns = [
    {
      ...
      cell: (row: TeacherQuest) => <span className="text-2xl">{row.icon}</span>,
    },
    {
      header: t("columns.title"),
      accessorFn: (row: TeacherQuest) => row.title,
      className: "font-medium",
    },
    {
      header: t("columns.category"),
      cell: (row: TeacherQuest) => <Badge variant="outline">{row.category}</Badge>,
    },
    {
      header: t("columns.frequency"),
      cell: (row: TeacherQuest) => (
        <Badge variant="secondary">{row.frequency}</Badge>
      ),
    },
    {
      header: t("columns.xp"),
      accessorFn: (row: TeacherQuest) => row.xpReward,
    },
    {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  <FormItem>
                    ...
                    >
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue
                            placeholder={t("form.selectFrequency")}
                          />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button
                type="submit"
                disabled={createQuest.isPending || updateQuest.isPending}
              >
