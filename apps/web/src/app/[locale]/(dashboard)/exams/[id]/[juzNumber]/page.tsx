...
export default function JuzHistoryPage({
  ...
  const { data: exams, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await api.get(
        `/exams?studentId=${studentId}&juzNumber=${juzNumber}`,
      );
      // Map backend entity to frontend interface
      return (res.data as ExamApiResponse[]).map(
        (e): ExamHistoryItem => ({
          id: e.id,
          date: e.date,
          score: e.finalScore,
  ...
  const bestScore =
    exams?.reduce((max, e) => (e.score && e.score > max ? e.score : max), 0) ||
    0;
              ...
              <TableBody>
                {exams.map((exam) => (
                  <TableRow key={exam.id}>
                    ...
                    <TableCell>
                      <span
                        className={`font-mono font-bold ${exam.passed ? "text-green-600" : "text-destructive"}`}
                      >
                        {exam.score !== null
                    ...
                    <TableCell className="text-right">
                      <Button
                        ...
                        onClick={() =>
                          router.push(
                            `/exams/${studentId}/${juzNumber}/${exam.id}`,
                          )
                        }
                      >
                        {t("viewDetails")}
