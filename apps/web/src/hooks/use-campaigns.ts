...
export function useCreateCampaign() {
  ...
  return useMutation({
    mutationFn: async (dto: CreateCampaignDto) => {
      const { data } = await api.post<Campaign>("/admin/campaigns", dto);
      return data;
    },
    onSuccess: () => {
...
export function useUpdateCampaign() {
  ...
  return useMutation({
    mutationFn: async ({ id, dto }: { id: string; dto: UpdateCampaignDto }) => {
      const { data } = await api.put<Campaign>(`/admin/campaigns/${id}`, dto);
      return data;
    },
...
export function useResetStreaks() {
  return useMutation({
    mutationFn: async (dto?: { mosqueId?: string; circleId?: string }) => {
      const { data } = await api.post<{ success: boolean; modifiedCount: number }>(
